...
def login_page():
    return send_from_directory(app_directory, 'login.html')  # Замените 'login.html' на фактическое имя файла, если оно другое

# Маршрут для отображения страницы регистрации
@app.route('/register_page')
def register_page():
    return send_from_directory(app_directory, 'register.html')  # Замените 'register.html' на фактическое имя файла
...
@jwt_required()  # Требуется токен для доступа
def get_books():
    # Пример списка книг, в реальной ситуации можешь получать его из базы данных
    books = [
